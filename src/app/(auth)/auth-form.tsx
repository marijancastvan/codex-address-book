...
export function AuthForm({ mode }: { mode: "login" | "register" }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (mode === "login") {
        ...
        router.replace(next?.startsWith("/") && !next.startsWith("//") ? next : "/contacts");
        router.refresh();
      } else {
        const { data: result, error: authError } = await supabase.auth.signUp({ email, password, options: { emailRedirectTo: `${window.location.origin}/auth/callback?next=/contacts` } });
        if (authError) throw authError;
        if (result.session) { router.replace("/contacts"); router.refresh(); }
  ...
  return <main className="auth-card">
    ...
    <p className="subtle">{mode === "login" ? <>Nemate nalog? <Link href="/register">Registrujte se</Link></> : <>Već imate nalog? <Link href="/login">Prijavite se</Link></>}</p>
  </main>;
}
